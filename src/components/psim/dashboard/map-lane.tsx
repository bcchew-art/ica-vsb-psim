...
import { SallyPortOverlay } from "./sally-port-overlay";
...
export function MapLane({ laneNumber, equipment }: Props) {
  ...
  const isSallyPortActive = usePsimStore((s) => s.sallyPortLanes.has(laneNumber));
  ...
  return (
    ...
    >
      ...
      <LaneContextMenu laneNumber={laneNumber} className="flex-1">
        <div className="flex-1 bg-slate-700 dark:bg-slate-700 h-[22px] rounded-sm relative shadow-[inset_0_2px_6px_rgba(0,0,0,0.5)] flex items-center justify-around px-space-3">
          ...
          {/* Sally port overlay (visible when active) */}
          <AnimatePresence>
            {isSallyPortActive && <SallyPortOverlay laneNumber={laneNumber} />}
          </AnimatePresence>
